...
export function getGridFromImage(imgData) {
  ...
  let medianArea = contourList
    .map((c) => cv.countourArea(c))
    .sort((a, b) => a - b)[Math.floor(contourList.size / 2)];
...
function fillClueNumbers(grid) {
  ...
  for (let j = 0; j < grid.length; j++) {
    for (let k = 0; k < grid[0].length; k++) {
      ...
      if (
        (!("top" in canGoTo) && "bottom" in canGoTo) ||
        (!("left" in canGoTo) && "right" in canGoTo)
      ) {
