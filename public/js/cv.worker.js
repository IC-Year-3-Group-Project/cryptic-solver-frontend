...
function getGridFromImage({ msg, payload }) {
  ...
  for (let i = 0; i < contours.size(); i++) {
    ...
    if (
      bounds.width < bounds.height * squareFactor ||
      bounds.width * squareFactor > bounds.height
    ) {
      continue;
    }
    ...
    if (
      contourArea < minArea ||
      contourArea < areaFactor * bounds.width * bounds.height
    ) {
      continue;
    }
  ...
  let medianArea = contourList
    .map((c) => cv.contourArea(c))
    .sort((a, b) => a - b)[Math.floor(contourList.length / 2)];
  let medianPerimeter = contourList
    .map((c) => cv.arcLength(c, true))
    .sort((a, b) => a - b)[Math.floor(contourList.length / 2)];

  temp = [];
  for (let c of contourList) {
    if (
      Math.abs(cv.contourArea(c) - medianArea) > medianArea / 2.0 &&
      Math.abs(cv.arcLength(c, true) - medianPerimeter) < medianPerimeter / 2.0
    ) {
      continue;
    }
    if (
      Math.abs(cv.arcLength(c, true) - medianPerimeter) >=
      medianPerimeter / 2.0
    ) {
      continue;
    }
